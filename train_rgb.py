...
class args:
    ...
    test_display = 1000
    weight_decay = 5e-4
    dataset = 'rgb_balance_v1'
    model_name = 'baseline-rgb-512-1117'
    batch_norm_decay = 0.95
    test_rgb_dir = 'rgb_balance_v1/test/images'
    test_thermal_dir = None
    test_label_dir = 'rgb_balance_v1/test/labels'
    multi_scale = True
    gpu_num = 0
    pretraining = False
    ckpt_step = 2000
...
with tf.Session(config=config) as sess:
    ...
    for step in range(1, args.num_steps+1):
        ...
        if (step in [50, 200]) or (step > 0 and step % args.test_display == 0):
            ...
            for test_rgb_file in os.listdir(args.test_rgb_dir):
                test_rgb_path = os.path.join(args.test_rgb_dir, test_rgb_file)
                if os.path.isfile(test_rgb_path) and test_rgb_file.endswith('.jpg'):
                    test_rgb_img = cv2.imread(test_rgb_path, cv2.IMREAD_COLOR)
