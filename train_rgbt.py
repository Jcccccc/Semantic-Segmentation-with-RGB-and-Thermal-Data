...
class args:
    ...
    model_name = 'rgbt-72f-wloss-1201'
    batch_norm_decay = 0.95
    test_rgb_dir = 'rgbt_balance/test/rgb'
    test_thermal_dir = 'rgbt_balance/test/thermal'
    test_label_dir = 'rgbt_balance/test/label'
    multi_scale = True
    gpu_num = 0
    pretraining = False
    ckpt_step = 5000
...
test_path_df = pd.read_csv(args.dataset+'/test_list.csv')
test_rgb_paths = test_path_df['rgb'].values
test_thermal_paths = test_path_df['thermal'].values 
test_label_paths = test_path_df['label'].values

dataset = DataSet(rgb_path=data_path_df['rgb'].values, 
                  thermal_path=data_path_df['thermal'].values, 
...
""" loss weights """
weights_per_class = tf.constant(
    [0.1, 0.1, 0.2, 10.0, 1.0, 10.0])
label_weighted = tf.gather(weights_per_class, label)
...
""" cross_entropy with weights """
cross_entropy = tf.reduce_mean(tf.multiply(
    tf.nn.sparse_softmax_cross_entropy_with_logits(logits=logits, labels=label), label_weighted))
...
with tf.Session(config=config) as sess:
    ...
    for step in range(1, args.num_steps+1):
        ...
        if (step in [50, 200]) or (step > 0 and step % args.test_display == 0):

            all_tests_result, avg_tests_result = dict(), dict()
            for i, test_rgb_path in enumerate(test_rgb_paths):
                #test_rgb_path = os.path.join(args.test_rgb_dir, test_rgb_file)
                if os.path.isfile(test_rgb_path):
                    test_rgb_img = cv2.resize(cv2.imread(test_rgb_path, cv2.IMREAD_COLOR),
                                              (args.input_size, args.input_size))
                    test_thermal_path = test_thermal_paths[i]
                    test_thermal_img = cv2.resize(cv2.imread(test_thermal_path, 0), 
                                                  (args.input_size, args.input_size)) \
                                          .reshape((args.input_size, args.input_size, 1))
                    test_img = np.concatenate((test_rgb_img, test_thermal_img), axis=2)
                    ...
                    #test_label_file = test_rgb_file.split('.')[0] + '.png'
                    #test_label_path = os.path.join(args.test_label_dir, test_label_file)
                    test_label_path = test_label_paths[i]
                    test_label = cv2.resize(np.array(Image.open(test_label_path)),
                                            (args.input_size, args.input_size))
